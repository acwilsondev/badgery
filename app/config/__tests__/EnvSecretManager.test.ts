...
describe('EnvSecretManager', () => {
  ...
  describe('Required Secrets', () => {
    test('should throw error when required secret is not found', async () => {
      ...
      await expect(secretManager.getSecret('API_KEY'))
        .rejects.toThrow('Required secret API_KEY not found');
    });
    ...
    test('should validate secret format when validator is provided', async () => {
      ...
      await expect(secretManager.getSecret('API_KEY', /^[A-Z0-9]{32}$/))
  ...
  describe('Optional Secrets', () => {
    test('should return default value for optional secret when not found', async () => {
      ...
      const value = await secretManager.getOptionalSecret('OPTIONAL_SECRET', 'default');
    ...
    test('should validate optional secret format when provided', async () => {
      ...
      await expect(secretManager.getOptionalSecret('OPTIONAL_KEY', 'default', /^[A-Z0-9]{8}$/))
  ...
  describe('Value Formatting', () => {
    ...
    test('should handle empty string after trimming', async () => {
      ...
      await expect(secretManager.getSecret('EMPTY_SECRET'))
        .rejects.toThrow('Required secret EMPTY_SECRET not found');
    });
  });
